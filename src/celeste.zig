...
pub fn _init(p8_api: P8API) void {
    for (&objects) |*obj| {
...
fn title_screen(p8_api: P8API) void {
    ...
    start_game_flash = 0;
    p8_api.music(40, 0, 7);
    load_room(7, 3, p8_api);
    //load_room(5, 2, p8_api);
}

fn begin_game(p8_api: P8API) void {
    frames = 0;
    seconds = 0;
    minutes = 0;
    music_timer = 0;
...
const Player = struct {
    ...
    fn init(self: *Player, common: *ObjectCommon, p8_api: P8API) void {
        self.p_jump = false;
        self.p_dash = false;
        self.grace = 0;
        self.jbuffer = 0;
        ...
        common.spr = 5;
        create_hair(&self.hair, common, p8_api);
    }

    fn update(self: *Player, common: *ObjectCommon, p8_api: P8API) void {
        ...
        // spikes collide
        if (spikes_at(common.x + common.hitbox.x, common.y + common.hitbox.y, common.hitbox.w, common.hitbox.h, common.spd.x, common.spd.y, p8_api)) {
            kill_player(self, common, p8_api);
            return;
        }
        ...
        const on_ground = common.is_solid(0, 1, p8_api);
        const on_ice = common.is_ice(0, 1, p8_api);

        // smoke particles
        if (on_ground and !self.was_on_ground) {
            init_object(EntityType.smoke, common.x, common.y + 4, p8_api);
        }
        ...
        if (self.dash_time > 0) {
            init_object(EntityType.smoke, common.x, common.y, p8_api);
            self.dash_time -= 1;
            common.spd.x = appr(common.spd.x, self.dash_target.x, self.dash_accel.x, p8_api);
            common.spd.y = appr(common.spd.y, self.dash_target.y, self.dash_accel.y, p8_api);
        } else {
            ...
            if (p8_api.abs(common.spd.x) > maxrun) {
                common.spd.x = appr(common.spd.x, sign(common.spd.x) * maxrun, deccel, p8_api);
            } else {
                common.spd.x = appr(common.spd.x, input * maxrun, accel, p8_api);
            }
            ...
            // wall slide
            if (input != 0 and common.is_solid(input, 0, p8_api) and !common.is_ice(input, 0, p8_api)) {
                maxfall = 0.4;
                if (p8_api.rnd(10) < 2) {
                    init_object(EntityType.smoke, common.x + input * 6, common.y, p8_api);
                }
            }

            if (!on_ground) {
                common.spd.y = appr(common.spd.y, maxfall, gravity, p8_api);
            }
            ...
            if (self.jbuffer > 0) {
                if (self.grace > 0) {
                    ...
                    // wall jump
                    var wall_dir: P8API.num = if (common.is_solid(3, 0, p8_api)) 1 else 0;
                    wall_dir = if (common.is_solid(-3, 0, p8_api)) -1 else wall_dir;
                    if (wall_dir != 0) {
                        psfx(2, p8_api);
                        self.jbuffer = 0;
                        common.spd.y = -2;
                        common.spd.x = -wall_dir * (maxrun + 1);
                        if (!common.is_ice(wall_dir * 3, 0, p8_api)) {
                            init_object(EntityType.smoke, common.x + wall_dir * 6, common.y, p8_api);
                        }
                    }
                }
            }
            ...
            self.spr_off += 0.25;
            if (!on_ground) {
                if (common.is_solid(input, 0, p8_api)) {
                    common.spr = 5;
                } else {
                    common.spr = 3;
                }
    ...
    fn draw(self: *Player, common: *ObjectCommon, p8_api: P8API) void {
        // clamp in screen
        if (common.x < -1 or common.x > 121) {
            common.x = clamp(common.x, -1, 121, p8_api);
            common.spd.x = 0;
        }

        set_hair_color(self.djump, p8_api);
        draw_hair(&self.hair, common, if (common.flip_x) -1 else 1, p8_api);
        p8_api.spr(common.spr, common.x, common.y, 1, 1, common.flip_x, common.flip_y);
        unset_hair_color(p8_api);
    }
};

fn psfx(x: P8API.num, p8_api: P8API) void {
    if (sfx_timer <= 0) {
        p8_api.sfx(x);
    }
}
...
fn create_hair(hair: []Hair, common: *ObjectCommon, p8_api: P8API) void {
    var i: P8API.num = 0;
    while (i <= 4) : (i += 1) {
...
fn set_hair_color(djump: P8API.num, p8_api: P8API) void {
    const col =
        if (djump == 1)
        8
    else
...
fn draw_hair(hair: []Hair, common: *ObjectCommon, facing: P8API.num, p8_api: P8API) void {
...
fn unset_hair_color(p8_api: P8API) void {
    p8_api.pal(8, 8);
}
...
const PlayerSpawn = struct {
    ...
    fn init(self: *PlayerSpawn, common: *ObjectCommon, p8_api: P8API) void {
        p8_api.sfx(4);
        common.spr = 3;
    ...
    fn update(self: *PlayerSpawn, common: *ObjectCommon, p8_api: P8API) void {
        if (self.state == 0) { // jumping up
            ...
            if (self.delay < 0) {
                ...
                init_object(EntityType.player, common.x, common.y, p8_api);
            }
        }
    }

    fn draw(self: *PlayerSpawn, common: *ObjectCommon, p8_api: P8API) void {
        set_hair_color(max_djump, p8_api);
        draw_hair(&self.hair, common, 1, p8_api);
        p8_api.spr(common.spr, common.x, common.y, 1, 1, common.flip_x, common.flip_y);
        unset_hair_color(p8_api);
    }
};
...
const Spring = struct {
    ...
    fn update(self: *Spring, common: *ObjectCommon, p8_api: P8API) void {
        if (self.hide_for > 0) {
            self.hide_for -= 1;
            if (self.hide_for <= 0) {
                common.spr = 18;
            ...
            if (hit_opt) |hit| {
                if (hit.common.spd.y >= 0) {
                    ...
                    if (below_opt) |below| {
                        break_fall_floor(&below.specific.fall_floor, &below.common, p8_api);
                    }
...
const Balloon = struct {
    ...
    //tile=22,
    fn init(self: *Balloon, common: *ObjectCommon, p8_api: P8API) void {
        self.offset = p8_api.rnd(1);
        self.start = common.y;
        self.timer = 0;
        common.hitbox = P8Rect{ .x = -1, .y = -1, .w = 10, .h = 10 };
    }
    fn update(self: *Balloon, common: *ObjectCommon, p8_api: P8API) void {
        if (common.spr == 22) {
            self.offset += 0.01;
    ...
    fn draw(self: *Balloon, common: *ObjectCommon, p8_api: P8API) void {
        if (common.spr == 22) {
...
const FallFloor = struct {
    ...
    fn update(self: *FallFloor, common: *ObjectCommon, p8_api: P8API) void {
        if (self.state == 0) { // idling
            if (common.check(EntityType.player, 0, -1) or common.check(EntityType.player, -1, 0) or common.check(EntityType.player, 1, 0)) {
                break_fall_floor(self, common, p8_api);
            }
    ...
    fn draw(self: *FallFloor, common: *ObjectCommon, p8_api: P8API) void {
        if (self.state != 2) {
            if (self.state != 1) {
...
fn break_fall_floor(self: *FallFloor, common: *ObjectCommon, p8_api: P8API) void {
    if (self.state == 0) {
        psfx(15, p8_api);
        self.state = 1;
...
const Smoke = struct {
    fn init(self: *Smoke, common: *ObjectCommon, p8_api: P8API) void {
        _ = self;
        common.spr = 29;
        common.spd.y = -0.1;
        ...
        common.flip_x = maybe(p8_api);
        common.flip_y = maybe(p8_api);
        common.solids = false;
    }
    fn update(self: *Smoke, common: *ObjectCommon, p8_api: P8API) void {
        _ = self;
        _ = p8_api;
        common.spr += 0.2;
        if (common.spr >= 32) {
...
const Fruit = struct {
    ...
    fn update(self: *Fruit, common: *ObjectCommon, p8_api: P8API) void {
        var hit_opt = common.collide(EntityType.player, 0, 0);
        if (hit_opt) |hit| {
            hit.specific.player.djump = max_djump;
            sfx_timer = 20;
            ...
            init_object(EntityType.life_up, common.x, common.y, p8_api);
            destroy_object(common);
            return;
        }
        self.off += 1;
...
const FlyFruit = struct {
    ...
    fn update(self: *FlyFruit, common: *ObjectCommon, p8_api: P8API) void {
        ...
        if (self.fly) {
            ...
            common.spd.y = appr(common.spd.y, -3.5, 0.25, p8_api);
            if (common.y < -16) {
                do_destroy = true;
            }
        } else {
    ...
    fn draw(self: *FlyFruit, common: *ObjectCommon, p8_api: P8API) void {
        var off: P8API.num = 0;
        if (!self.fly) {
...
const LifeUp = struct {
    ...
    fn update(self: *LifeUp, common: *ObjectCommon, p8_api: P8API) void {
        _ = p8_api;
        self.duration -= 1;
        if (self.duration <= 0) {
            destroy_object(common);
    ...
    fn draw(self: *LifeUp, common: *ObjectCommon, p8_api: P8API) void {
        self.flash += 0.5;
...
const FakeWall = struct {
    fn update(self: *FakeWall, common: *ObjectCommon, p8_api: P8API) void {
        _ = self;
        ...
        if (hit_opt) |hit| {
            if (hit.specific.player.dash_effect_time > 0) {
                ...
                destroy_object(common);
                init_object(EntityType.smoke, common.x, common.y, p8_api);
                init_object(EntityType.smoke, common.x + 8, common.y, p8_api);
                init_object(EntityType.smoke, common.x, common.y + 8, p8_api);
                init_object(EntityType.smoke, common.x + 8, common.y + 8, p8_api);
                init_object(EntityType.fruit, common.x + 4, common.y + 4, p8_api);
                return; //
            }
        }
    ...
    fn draw(self: *FakeWall, common: *ObjectCommon, p8_api: P8API) void {
        _ = self;
...
const Key = struct {
    // tile=8,
    // if_not_fruit=true,
    fn update(self: *Key, common: *ObjectCommon, p8_api: P8API) void {
        _ = self;
...
const Chest = struct {
    ...
    fn update(self: *Chest, common: *ObjectCommon, p8_api: P8API) void {
        if (has_key) {
            self.timer -= 1;
            ...
            if (self.timer <= 0) {
                sfx_timer = 20;
                p8_api.sfx(16);
                init_object(EntityType.fruit, common.x, common.y - 4, p8_api);
                destroy_object(common);
            }
        }
    }
...
const Platform = struct {
    ...
    fn update(self: *Platform, common: *ObjectCommon, p8_api: P8API) void {
        ...
        if (!common.check(EntityType.player, 0, 0)) {
            ...
            if (hit_opt) |hit| {
                hit.common.move_x(common.x - self.last, 1, p8_api);
            }
        }
        self.last = common.x;
    }

    fn draw(self: *Platform, common: *ObjectCommon, p8_api: P8API) void {
        _ = self;
...
const Message = struct {
    ...
    fn draw(self: *Message, common: *ObjectCommon, p8_api: P8API) void {
...
const BigChest = struct {
    ...
    fn draw(self: *BigChest, common: *ObjectCommon, p8_api: P8API) void {
        if (self.state == 0) {
            var hit_opt = common.collide(EntityType.player, 0, 8);
            if (hit_opt) |hit| {
                if (hit.common.is_solid(0, 1, p8_api)) {
            ...
            if (self.timer < 0) {
                ...
                flash_bg = false;
                new_bg = true;
                init_object(EntityType.orb, common.x + 4, common.y + 4, p8_api);
                pause_player = false;
            }
            for (&self.particles) |*p| {
                p.y += p.spd.y;
...
const Orb = struct {
    ...
    fn draw(self: *Orb, common: *ObjectCommon, p8_api: P8API) void {
        _ = self;
        common.spd.y = appr(common.spd.y, 0, 0.5, p8_api);
        var hit_opt = common.collide(EntityType.player, 0, 0);
        if (hit_opt) |hit| {
            if (common.spd.y == 0) {
                music_timer = 45;
...
const Flag = struct {
    ...
    fn draw(self: *Flag, common: *ObjectCommon, p8_api: P8API) void {
...
const RoomTitle = struct {
    ...
    fn draw(self: *RoomTitle, common: *ObjectCommon, p8_api: P8API) void {
        self.delay -= 1;
        if (self.delay < -30) {
            destroy_object(common);
        } else if (self.delay < 0) {
...
const ObjectCommon = struct {
    ...
    fn is_ice(self: *ObjectCommon, ox: P8API.num, oy: P8API.num, p8_api: P8API) bool {
        return ice_at(self.x + self.hitbox.x + ox, self.y + self.hitbox.y + oy, self.hitbox.w, self.hitbox.h, p8_api);
    }

    fn move(self: *ObjectCommon, ox: P8API.num, oy: P8API.num, p8_api: P8API) void {
        var amount: P8API.num = 0;

        self.rem.x += ox;
        amount = p8_api.flr(self.rem.x + 0.5);
        self.rem.x -= amount;
        self.move_x(amount, 0, p8_api);

        self.rem.y += oy;
        amount = p8_api.flr(self.rem.y + 0.5);
        self.rem.y -= amount;
        self.move_y(amount, p8_api);
    }

    fn move_x(self: *ObjectCommon, amount: P8API.num, start: P8API.num, p8_api: P8API) void {
        if (self.solids) {
            const step = sign(amount);
            var i: P8API.num = start;
            while (i <= p8_api.abs(amount)) : (i += 1) { // i <= amount
                if (!self.is_solid(step, 0, p8_api)) {
                    self.x += step;
                } else {
                    self.spd.x = 0;
                    self.rem.x = 0;
    ...
    fn move_y(self: *ObjectCommon, amount: P8API.num, p8_api: P8API) void {
        if (self.solids) {
            const step = sign(amount);
            var i: P8API.num = 0;
            while (i <= p8_api.abs(amount)) : (i += 1) {
                if (!self.is_solid(0, step, p8_api)) {
                    self.y += step;
                } else {
                    self.spd.y = 0;
                    self.rem.y = 0;
    ...
    fn is_solid(self: *ObjectCommon, ox: P8API.num, oy: P8API.num, p8_api: P8API) bool {
        if (oy > 0 and !self.check(EntityType.platform, ox, 0) and self.check(EntityType.platform, ox, oy)) {
            return true;
        }
        return solid_at(self.x + self.hitbox.x + ox, self.y + self.hitbox.y + oy, self.hitbox.w, self.hitbox.h, p8_api) or self.check(EntityType.fall_floor, ox, oy) or self.check(EntityType.fake_wall, ox, oy);
    }
};
...
fn init_object(etype: EntityType, x: P8API.num, y: P8API.num, p8_api: P8API) void {
    _ = create_object(etype, x, y, p8_api);
}

fn create_object(etype: EntityType, x: P8API.num, y: P8API.num, p8_api: P8API) *Object {
...
fn next_room(p8_api: P8API) void {
    ...
    if (room.x == 7) {
        load_room(0, room.y + 1, p8_api);
    } else {
        load_room(room.x + 1, room.y, p8_api);
    }
}

fn load_room(x: P8API.num, y: P8API.num, p8_api: P8API) void {
    has_dashed = false;
    has_key = false;
    ...
    while (tx <= 15) : (tx += 1) {
        ...
        while (ty <= 15) : (ty += 1) {
            ...
            if (tile == 11) {
                var p = create_object(EntityType.platform, tx * 8, ty * 8, p8_api);
                ...
                for (all_entity_types) |et| {
                    if (tile == @intFromEnum(et)) {
                        init_object(et, tx * 8, ty * 8, p8_api);
                    }
                }
            }
        }
    ...
    if (!is_title()) {
        init_object(EntityType.room_title, 0, 0, p8_api);
    }
}
...
pub fn _update(p8_api: P8API) void {
    ...
    if (will_restart and delay_restart > 0) {
        ...
        if (delay_restart <= 0) {
            will_restart = false;
            load_room(room.x, room.y, p8_api);
        }
    }

    // update each object
    for (&objects) |*obj| {
        update_object(obj, p8_api);
    }
...
pub fn _draw(p8_api: P8API) void {
    if (freeze > 0) {
        return;
    }
...
fn draw_object(object: *Object, p8_api: P8API) void {
    if (object.common.active) {
        switch (object.specific) {
            EntityType.balloon => |*b| {
                b.draw(&object.common, p8_api);
            },
            EntityType.big_chest => |*b| {
                b.draw(&object.common, p8_api);
            },
            EntityType.fall_floor => |*ff| {
                ff.draw(&object.common, p8_api);
            },
            EntityType.fake_wall => |*fw| {
                fw.draw(&object.common, p8_api);
            },
            EntityType.flag => |*f| {
                f.draw(&object.common, p8_api);
            },
            EntityType.fly_fruit => |*ff| {
                ff.draw(&object.common, p8_api);
            },
            EntityType.life_up => |*lu| {
                lu.draw(&object.common, p8_api);
            },
            EntityType.message => |*m| {
                m.draw(&object.common, p8_api);
            },
            EntityType.orb => |*o| {
                o.draw(&object.common, p8_api);
            },
            EntityType.platform => |*p| {
                p.draw(&object.common, p8_api);
            },
            EntityType.player_spawn => |*ps| {
                ps.draw(&object.common, p8_api);
            },
            EntityType.player => |*p| {
                p.draw(&object.common, p8_api);
            },
            EntityType.room_title => |*rt| {
                rt.draw(&object.common, p8_api);
            },
...
fn draw_time(x: P8API.num, y: P8API.num, p8_api: P8API) void {
...
fn kill_player(player: *Player, common: *ObjectCommon, p8_api: P8API) void {
    _ = player;
    sfx_timer = 12;
    p8_api.sfx(0);
    deaths += 1;
...
fn update_object(object: *Object, p8_api: P8API) void {
    if (object.common.active) {
        object.common.move(object.common.spd.x, object.common.spd.y, p8_api);
        switch (object.specific) {
            EntityType.balloon => |*b| {
                b.update(&object.common, p8_api);
            },
            EntityType.big_chest => |_| {},
            EntityType.chest => |*c| {
                c.update(&object.common, p8_api);
            },
            EntityType.fall_floor => |*ff| {
                ff.update(&object.common, p8_api);
            },
            EntityType.fake_wall => |*fw| {
                fw.update(&object.common, p8_api);
            },
            EntityType.flag => |_| {},
            EntityType.fly_fruit => |*ff| {
                ff.update(&object.common, p8_api);
            ...
            EntityType.fruit => |*f| {
                f.update(&object.common, p8_api);
            },
            EntityType.key => |*k| {
                k.update(&object.common, p8_api);
            },
            EntityType.life_up => |*lu| {
                lu.update(&object.common, p8_api);
            },
            EntityType.message => |_| {},
            EntityType.orb => |_| {},
            EntityType.platform => |*p| {
                p.update(&object.common, p8_api);
            },
            EntityType.player_spawn => |*ps| {
                ps.update(&object.common, p8_api);
            },
            EntityType.player => |*p| {
                p.update(&object.common, p8_api);
            },
            EntityType.smoke => |*s| {
                s.update(&object.common, p8_api);
            },
            EntityType.spring => |*s| {
                s.update(&object.common, p8_api);
            },
...
fn tile_at(x: P8API.num, y: P8API.num, p8_api: P8API) P8API.tile {
    return p8_api.mget(room.x * 16 + x, room.y * 16 + y);
}

fn spikes_at(x: P8API.num, y: P8API.num, w: P8API.num, h: P8API.num, xspd: P8API.num, yspd: P8API.num, p8_api: P8API) bool {
...
fn tile_flag_at(x: P8API.num, y: P8API.num, w: P8API.num, h: P8API.num, flag: P8API.num, p8_api: P8API) bool {
    var i: P8API.num = p8_api.max(0, @divTrunc(x, 8));
    while (i <= p8_api.min(15, (x + w - 1) / 8)) : (i += 1) {
        var j = p8_api.max(0, @divTrunc(y, 8));
        while (j <= p8_api.min(15, (y + h - 1) / 8)) : (j += 1) {
            if (p8_api.fget(@intCast(tile_at(i, j, p8_api)), flag)) {
                return true;
            }
        }
    }
...
fn maybe(p8_api: P8API) bool {
    return p8_api.rnd(1) < 0.5;
}

fn solid_at(x: P8API.num, y: P8API.num, w: P8API.num, h: P8API.num, p8_api: P8API) bool {
    return tile_flag_at(x, y, w, h, 0, p8_api);
}

fn ice_at(x: P8API.num, y: P8API.num, w: P8API.num, h: P8API.num, p8_api: P8API) bool {
    return tile_flag_at(x, y, w, h, 4, p8_api);
}

fn clamp(x: P8API.num, a: P8API.num, b: P8API.num, p8_api: P8API) P8API.num {
    return p8_api.max(a, p8_api.min(b, x));
}

fn appr(val: P8API.num, target: P8API.num, amount: P8API.num, p8_api: P8API) P8API.num {
